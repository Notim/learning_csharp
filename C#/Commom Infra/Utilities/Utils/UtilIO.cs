...
namespace UTIL.Utils {
    ...
    public static class UtilIO {
        ...
        public static FileInfo createFile(string pathFolder, string fileName, bool combineDefaultFolder = true) {
            ...
            var F = new FileInfo(Path.Combine(pathFolder, fileName));
            var Fs = F.Create();
        ...
        public static string copiarArquivo(FileInfo OFile,
                                           string   pathNovoDestino,
                                           string   nomeArquivo,
                                           bool     renomearSeExistir = true) {
            if (!OFile.Exists) {
                return "";
            }
            ...
            if (renomearSeExistir) {
                ...
                while (File.Exists(nomeArquivoCompleto)) {
                    string prefixo = String.Concat("(", cont.ToString()
                                                            .PadLeft(3, '0'), ")"
                                                  );

                    if (nomeArquivo.Contains(prefixo)) {
                        nomeArquivo = nomeArquivo.Replace(prefixo, String.Concat("(", (++cont).ToString()
                                                                                              .PadLeft(3, '0'), ")"
                                                                                )
                                                         );
