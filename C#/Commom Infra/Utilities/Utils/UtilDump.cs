    ...
    public static class UtilDump {
        ...
        private static void dump(object obj, int tab) {
            ...
            if (obj is IEnumerable enumerable && !(enumerable is string)) {
                dump_enumerable(enumerable, tab);
        ...
        private static string dump_write(int num, string str) {
            return "<pre>" +((new string(' ', num * 2) + str)) + "</pre>";
        }
        ...
        public static void displayProperties(object objectA, string objName = "") {
            ...
            foreach (var propertyInfo in objectType.GetProperties(BindingFlags.Public | BindingFlags.Instance).Where(x => x.CanRead)) {
