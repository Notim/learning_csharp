...
namespace BLL.Request {
    ...
    public class RequestAsync : IRequestAsync {
        ...
        public async Task<TextReader> doRequestAsync(WebRequest OWebRequest) {
            var ConnectTask = Task.Factory.FromAsync((cb, objeto) => ((HttpWebRequest) objeto).BeginGetResponse(cb, objeto), res => ((HttpWebRequest) res.AsyncState).EndGetResponse(res), OWebRequest);
